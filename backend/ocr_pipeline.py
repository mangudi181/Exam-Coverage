...
#  Handles:
#    1.  1)  1:  Q1  Q.1  Q.No.1  1(a)  1a)  (1)  i.  ii.  (i)
QUESTION_NO_PATTERNS = re.compile(
    ...
    r'|\d{1,2}\s*(?:\(\s*[a-z]\s*\)|[a-z]\s*\))' # 1(a) 1a) 2(b) 2b)
    r'|\d{1,3}\s*[.):,]\s*'        # 1. 1) 1: 1,
    r'|\(\s*\d{1,3}\s*\)'         # (1) (123)
    r'|[a-hA-H]\s*[.),]\s*'       # a. b) a,
    r')',
...
INLINE_BLOOM_TAG = re.compile(r'[\(\[]\s*(K[1-6I]|BTL\s*\d)\s*[\)\]]|[,.\s]*\b(K[1-6I])\b', re.IGNORECASE)
...
def extract_blooms(text: str) -> Optional[str]:
    ...
    m = re.search(r'\b(K[1-6I])\b', text, re.IGNORECASE)
    if m:
        val = m.group(1).upper()
        return "K1" if val == "KI" else val
...
def remove_marking_artifacts(text: str) -> str:
    ...
    # 3. Matches standalone CO indicators like: CO2, CO3, CO4, CO5, COs, COS
    text = re.sub(
...
def preprocess_raw_text(text: str) -> str:
    ...
    text = re.sub(r'([.?!,])\s*(\d{1,2}\s*[a-z]\s*\)\s+[A-Z])', r'\1\n\2', text)
    
    # 3. Handle cases where the number is just merged without punctuation but clear: "example 7. What" or "impact 15a) Describe"
    text = re.sub(r'([a-zA-Z0-9])\s*[.?!,]\s*(\d{1,2}\s*[.):,])', r'\1.\n\2', text)
    text = re.sub(r'([a-zA-Z0-9])\s+(\d{1,2}\s*[a-z]\s*\)\s+[A-Z])', r'\1\n\2', text)
...
def post_process_questions(questions: List[Dict]) -> List[Dict]:
    ...
    for q in questions:
        ...
        split_pattern = r'([\.?!,])\s*(\d{1,2}\s*[.):,]\s+[A-Z]|\(\s*[a-z]\s*\)\s+[A-Z]|\d{1,2}\s*[a-z]\s*\)\s+[A-Z])'
        
        # Also catch aggressive unpunctuated merges like "reach and impact 15a) Describe"
        if not re.search(split_pattern, text):
            split_pattern = r'([a-z])\s+(\d{1,2}\s*[a-z]\s*\)\s+[A-Z])'
        ...
        if re.search(split_pattern, text):
            ...
            for i in range(1, len(sub_parts), 2):
                ...
                sep_match = re.search(r'([a-z\.?!,])\s+(\d{1,2}\s*(?:\(\s*[a-z]\s*\)|[a-z]\s*\)|[.):,]*)\s*)([A-Z].*)', separator + payload)
                if sep_match:
                    ...
                    new_q_no = sep_match.group(2).strip()
...
QTEXT_STRIP = [
    ...
    re.compile(r'\b\d+\s*[Mm]arks?\b'),        # 2 marks
    re.compile(r'\(\s*(K[1-6I]|BTL\s*\d)\s*\)', re.I),  # (K2), (BTL3), (KI)
    re.compile(r'\[\s*(K[1-6I]|BTL\s*\d)\s*\]', re.I),  # [K2], [KI]
    re.compile(r'[,.\s]*\b(K[1-6I])\b', re.I),          # standalone K2, KI
...
def clean_question_text(text: str) -> str:
    ...
    # User's requested rule to strictly remove trailing Bloom tags and punctuation
...
def is_junk_question(qno: str, qtext: str, marks, blooms) -> bool:
    ...
    if any(phrase.lower() in text_lower for phrase in HEADER_PHRASES) or "degree examinations" in text_lower or "common to all branches" in text_lower or "common to all departments" in text_lower or "b.tech" in text_lower:
        # If it has heavy header keywords, it's junk even if the AI assigned a qno
        return True
